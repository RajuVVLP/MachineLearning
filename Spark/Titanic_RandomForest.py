...
#Definind a RandomForest Classifier with the column to be predicted-SurvivedIndexed and features-Features as inputs.
rf = RandomForestClassifier(labelCol="SurvivedIndexed", featuresCol="Features")
...
pipeline = Pipeline(stages=[PClassIndexer,SexIndexer,EmbarkedIndexer, labelIndexer,assembler, rf,labelConverter])
...
paramGrid = ParamGridBuilder().addGrid(rf.maxBins, [10,15,20]).addGrid(rf.maxDepth, [3,5,7,9]).addGrid(rf.impurity, ["entropy", "gini"]).build()

#This is used to evaluate the model based on metrics. There are 3 nuilt-in evaluators - For Regression, For Binary Classification and the other for Multiclass Classification.
...
#This gives the avg metrics for all the combinations in paramgrid. no of maxbins * no of max depths * no of impurity
#in the above case, it is 3*4*2, which is 24.
...
prediction_new.repartition(1).write.format('com.databricks.spark.csv').option("header", "true").save(PATH+'/Submission_RandomForest')
